...
class UnitedEnergy():
    ...
    def __login(self):
        ...
        login_response = self.session.post(f"{self.apibase}login_security_check", data=payload)

        login_response.raise_for_status()
        if("You are logged in as".encode() not in login_response.content):
            raise Exception("Couldnt validate the login")
        if("The credentials entered were not recognised".encode() in login_response.content):
            raise Exception("Invalid Login")

    def __fetch_usage_data(self, reportperiod: enums.ReportPeriod, periodoffset: enums.PeriodOffset):
    ...
    def get_meters(self):
        meters_array = []
        response_data = self.fetch_recent_usage_data(enums.ReportPeriod.year, enums.PeriodOffset.current)
    ...
    def fetch_and_print_most_recent_usage_data(self, reportperiod: enums.ReportPeriod, periodoffset: enums.PeriodOffset):
        response_data = self.fetch_recent_usage_data(reportperiod, periodoffset)
        self.__print_usage_data(response_data)

    def fetch_recent_usage_data(self, reportperiod: enums.ReportPeriod, periodoffset: enums.PeriodOffset):
    ...
    def fetch_recent_usage_data_and_reformat(self, reportperiod: enums.ReportPeriod, periodoffset: enums.PeriodOffset):
        return self.__create_usage_data_array(self.fetch_recent_usage_data(reportperiod, periodoffset), reportperiod)

    def fetch_last_reading(self, reportperiod: enums.ReportPeriod, periodoffset: enums.PeriodOffset):
        data_to_filter = self.fetch_recent_usage_data_and_reformat(reportperiod, periodoffset)
        filtered_results = self.__filter_zero_kwh(data_to_filter)
        return filtered_results[-1]

    def fetch_cumilitive_reading(self, reportperiod: enums.ReportPeriod, periodoffset: enums.PeriodOffset):
        ...
        for index, value in enumerate(usage_data["selectedPeriod"]["consumptionData"]["peak"]):
            if value["total"] == 0: #This may not work if you actually use no power at all for an interval, most houses will have continual load.
                break

            response_data["total"] += value["total"]
            response_data["timestamp"] = self.__derive_timestamp_from_array_position(reportperiod, collection_date, index)
            response_data["price"] += usage_data["selectedPeriod"]["costData"]["peak"][index]["total"]
            self.__log_msg("New total {0} {1}".format(response_data["total"], value["total"]))
    ...
    def __extract_collection_timestamp(self, data, reportperiod: enums.ReportPeriod):
        date = {}
        subtitle = data["selectedPeriod"]["subtitle"]
        if reportperiod == enums.ReportPeriod.day:
            date = datetime.strptime(subtitle, "%A %d %B %Y")
        elif reportperiod == enums.ReportPeriod.month:
            date = datetime.strptime(subtitle, "%B %Y")
        elif reportperiod == enums.ReportPeriod.year:
            date = datetime.strptime(subtitle, "%Y")
        else:
            raise Exception("Invalid Report Period")
        return date

    def __derive_timestamp_from_array_position(self, reportperiod: enums.ReportPeriod, collection_date, array_position):
        timestamp = ""
        if reportperiod == enums.ReportPeriod.day:
            timestamp = "{0}-{1}-{2} {3:02d}:00".format(collection_date.year, collection_date.month, collection_date.day, array_position)
        elif reportperiod == enums.ReportPeriod.month:
            timestamp = "{0}-{1}-{2}".format(collection_date.year, collection_date.month, array_position)
        elif reportperiod == enums.ReportPeriod.year:
    ...
    def __create_usage_data_array(self, api_data, reportperiod: enums.ReportPeriod):
        response_array = []
        collection_date = self.__extract_collection_timestamp(api_data, reportperiod)

        for index, value in enumerate(api_data["selectedPeriod"]["consumptionData"]["peak"]):
            timestamp = self.__derive_timestamp_from_array_position(reportperiod, collection_date, index)
            response_data = {"timestamp": timestamp, "total": value["total"], "price": api_data["selectedPeriod"]["costData"]["peak"][index]["total"] }
